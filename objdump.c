...
static const char * disasm_syms = NULL;    /* File with disassembly start symbols  */
...
usage (FILE *stream, int status)
{
  ...
  fprintf (stream, _("\
      --disassemble_file=<filename> Display assembler contents from symbols listed in file \'filename'\n"));
  fprintf (stream, _("\
...
{"disassemble_file", optional_argument, NULL, 'k'},
...
static void
disassemble_section_alt (bfd *abfd, asection *section, void *inf, char **names, int size)
{
  const struct elf_backend_data *bed;
  bfd_vma sign_adjust = 0;
  struct disassemble_info *pinfo = (struct disassemble_info *) inf;
        ...
        if (streq (name, names[i]))
          ...
          if (sym->flags & BSF_FUNCTION)
          {
                ...
                && ((elf_symbol_type *) sym)->internal_elf_sym.st_size > 0)
            {
              ...
              //loop_until = stop_offset_reached;
              loop_until = next_sym;
              //nextstop_offset = addr_offset + ((elf_symbol_type *) sym)->internal_elf_sym.st_size;
            }
            else
            {
              /* Otherwise we need to tell the loop heuristic to
...
disassemble_data (bfd *abfd)
{
  ...
  if(disasm_syms != NULL){
    FILE * fh = fopen(disasm_syms, "r");
#define MAX_LINE 255
    char rline[MAX_LINE] = { 0 };
    if(!fh){
      perror(disasm_syms);
      return;
    }
    ...
    char ** names;
    int size = 0;
    while(fgets(rline, MAX_LINE, fh) != NULL){
      size++;
    }
    rewind(fh);
    ...
    while(fgets(rline, MAX_LINE, fh) != NULL){
      ...
      names[idx] = malloc(strlen(rline)+1);
      strcpy(names[idx], rline);
      idx++;
    }
    fclose(fh);
