...
//Moved from fadvise.c
void fadvise (FILE * fp, fadvice_t advice) {
  posix_fadvise(fileno(fp), 0 , 0 , advice);
}
...
#ifdef QSYM_TEST
  //For whatever reason, freopen doesn't appear to properly
  //redirect the file input into stdin for the qsym environment.
  //So we just use fopen instead and pass that file pointer
  FILE * fl = fopen("./tsortFile", "r");
#endif
  ...
  while (1)
    {
      /* T2. Next Relation.  */
#ifdef QSYM_TEST
      size_t len = readtoken (fl, DELIM, sizeof (DELIM) - 1, &tokenbuffer);
#else
