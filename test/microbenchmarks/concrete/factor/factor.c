...
//Def from quote.h. Trivial implementation suffices for our purposes.
char const *quote (char const *arg) {
  return arg;
}  

//We don't need the verify type tests for the TASE evaluation.
#define verify(x)  //
...
//Believe the "_" function is used in gettext.h or somewhere similar for localization support.
const char * _(const char * input) {
  return input;
}

//Since we're not using coreutils' normal die macro, no need to define the
//quoted filename macro.
#define quotef(filename) filename

//Simplified die implementation to replace that in die.h.
#define die(status, ... ) \
  { printf("Died in tsort with error code %d \n", status); exit(-1); }
...
void error (int __status, int __errnum, const char *__format, ...) {
  printf("Encountered error status %d and errno %d. Exiting. \n", __status, __errnum);
  exit(-1);
}
char *
...
//We've removed the option macro dependencies for the TASE tests
#if 0
...
//Don't need usage for the TASE benchmarks.
#if 0
...
//We use factorEntryShim for the TASE tests to simplify 
#if 0
int
...
void factorEntryShim (const char * num) {
  lbuf_alloc();
  print_factors(num);
}
