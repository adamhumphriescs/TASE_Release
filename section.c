...
void
bfd_map_over_sections_alt (bfd *abfd,
			   void (*operation) (bfd *, asection *, void *, char**, int),
			   void *user_storage, char** names, int size)
{
  asection *sect;
  unsigned int i = 0;

  for (sect = abfd->sections; sect != NULL; i++, sect = sect->next)
    (*operation) (abfd, sect, user_storage, names, size);
